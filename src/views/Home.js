  ...
  async function submitHandler(e) {
    ...
    try {
      const response = await axios.post(
        `${process.env.REACT_APP_API_URL}api/v1/moviegoers`,
        { email }
      );
    ...
    <Fragment>
      ...
      <div style={{ backgroundColor: "#F5F6F8" }} className="py-5">
        <Container>
          <Row className="mb-4">
            <Col>
              <h5 className="font-weight-bold text-primary header-now-showing__home">
                Now Showing
              </h5>
            </Col>
            <Col className="d-flex justify-content-end">
              <Link className="text-primary text-decoration-none" to="/movies">
                view all
              </Link>
            </Col>
          </Row>
          <div className="now-showing__home landscape-list__home">
            {nowShowingMovies.map((element, index) => {
              return (
                <Card key={index.toString()} className="mr-3">
                  <Card.Body>
                    <img
                      width="180"
                      src={element.picture}
        ...
        <Container>
            ...
            {seasons.map((element, index) => {
              return (
                <Button
                  ...
                  variant={
                    selectedSeason === element ? "primary" : "outline-primary"
                  }
                  className="mr-2 py-2"
                >
                  {element}
          ...
          <div className="upcoming-movies__home landscape-list__home">
            {loading ? (
              ...
              <div>There is no films at this season.</div>
            )}
          </div>
        </Container>
      </div>
            ...
            <Card.Body>
              <div className="d-flex justify-content-center align-items-center flex-column text-center">
                ...
                <h1
                  ...
                  className="display-6 text-primary font-weight-bold"
                >
                  Moviegoers
                </h1>
                <Form
                  onSubmit={submitHandler}
                  className="row mt-3 mb-4 d-flex justify-content-center"
                >
