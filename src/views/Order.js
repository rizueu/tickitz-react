    ...
    const getSoldSeat = async () => {
      try {
        const response = await http(token).get(
          `api/v1/soldseats?showTimeId=${location.showTimeId}`
        );
        setSoldSeat((current) => {
          return [...current, ...response.data.results];
        });
  ...
  if (method === "booking") {
    ...
    const selectedSeats = (e) => {
      ...
      if (typeof values === "string") {
        ...
        if (
          seats.indexOf(values[0]) === -1 &&
          seats.indexOf(values[1]) === -1
        ) {
        ...
        <div className="bg-primary py-4" id="choose-movie">
          <Container>
            <Row>
              ...
              >
                <span className="text-white">{results.movieTitle}</span>
            ...
            <div className="row mt-4 d-none d-md-flex">
              <div className="col">
                <button
                  onClick={chooseMovie}
                  style={{ fontSize: "15px" }}
                  className="btn btn-outline-primary rounded-3 py-3 w-100"
                >
                  Choose your movie
                </button>
              </div>
              <div className="col">
                <Link
                  to="/ticket/payment"
                  style={{ fontSize: "15px" }}
                  className="btn btn-primary rounded-3 py-3 w-100"
                >
                  Checkout now
                </Link>
              </div>
            ...
            <Card style={{ borderRadius: "1rem" }} className="border-0">
              ...
              >
                <div className="text-center mb-4">
                  <img src={results.picture} alt="cinemas" />
                  <h4 className="mt-2">{results.cinemaName}</h4>
                </div>
    ...
    const handleInput = (e, prop) => {
      dispatch(setPersonalInfo(prop, e.target.value));
    };

    const getTicket = () => {
      if (!results.fullName || !results.email || !results.phone) {
        dispatch(setMessage("Form can't be empty", "warning"));
      } else if (
        results.phone.match(/[a-z]/gi) ||
        results.phone.match(/[^0-9]/gi)
      ) {
        dispatch(setMessage("Incorrect phone number", "warning"));
      } else {
        dispatch(setMessage(null, null));
        http(token).post("api/v1/transaction", {
          ...
          cinemaCity: results.cinemaCity,
          paymentMethod: results.paymentMethod,
          totalPayment: results.totalPayment,
        });
        history.push("/ticket/payment/result");
      }
    };
                ...
                <form>
                  ...
                  <div className="input-group mb-4">
                    ...
                    <input
                      ...
                      className="form-control rounded-end px-3 py-2"
                    />
                  </div>
                  {results.message ? (
                    <div
            ...
            <div style={{ borderRadius: "1rem" }} className="card border-0">
              <div className="card-body p-4">
                <div className="d-flex flex-wrap align-items-center text-center mb-4">
                  <button
                    className="btn btn-payment-method"
                    onClick={() => dispatch(setPaymentMethod("Google Pay"))}
                  >
                    <svg
                      width="74"
                      height="30"
                  ...
                  <button
                    className="btn btn-payment-method"
                    onClick={() => dispatch(setPaymentMethod("GoPay"))}
                  >
                    <svg
                      width="106"
                      height="35"
                  ...
                  <button
                    className="btn btn-payment-method"
                    onClick={() => dispatch(setPaymentMethod("Dana"))}
                  >
                    <img src={logoDana} width="90" alt="dana-payment-method" />
                  </button>
                  <button
                    className="btn btn-payment-method"
                    onClick={() => dispatch(setPaymentMethod("BCA"))}
                  >
                    <svg
                      width="85"
                      height="28"
                  ...
                  <button
                    className="btn btn-payment-method"
                    onClick={() => dispatch(setPaymentMethod("BRI"))}
                  >
                    <svg
                      width="45"
                      height="38"
                  ...
                  <button
                    className="btn btn-payment-method"
                    onClick={() => dispatch(setPaymentMethod("OvO"))}
                  >
                    <svg
                      width="92"
                      height="30"
